...
// 해시 테이블의 원소를 연속적으로 가져올때
//position은 다음 노드의 주소를 받아오도록 설계되어 있음.
//pkey pValue 에는 현재 노드의 키와 밸류를 전달.
...
int hashDestroy(LPHASH lpHash)
{
	...
	for (i=0;i<lpHash->nHashSize;i++) {
		...
		while (NULL != lpNode){
			...
			//노드 메모리를 제거한다.
			free(lpTemp); /* 포인트 lpTemp를 삭제했어야 함. lpNode를 삭제함으로써 이상해짐.....*/
		}
