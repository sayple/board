...
int menuRun(LPC_MENU lpMenu,int sd,LPARRAY userFullList,int* chatUser)
{
    int nErr;
    int i, menu;
    ...
    while (1) {
        ...
        sprintf(buffer,"│                                                                                                                                       │\n");
        send(sd,buffer,strlen(buffer),0);
        textAlign(140,buffer,lpMenu->mainTitle);
        send(sd,buffer,strlen(buffer),0);
        sprintf(buffer,"│                                                                                                                                       │\n");
        send(sd,buffer,strlen(buffer),0);
        send(sd,buffer,strlen(buffer),0);
        ...
        for (i=0;i<lpMenu->menuCount;i++) {
                textAlign(140,buffer,menuItem[i].menuTitle);
            	send(sd,buffer,strlen(buffer),0);
        }
        sprintf(endText,"[%d] END", i+1);
        textAlign(140,buffer,endText);
        send(sd,buffer,strlen(buffer),0);
        sprintf(buffer,"│                                                                                                                                       │\n");
        for(int i =lpMenu->menuCount+7; i<15;i++){
             send(sd,buffer,strlen(buffer),0);
        }
        sprintf(buffer,"└───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘\n");
        send(sd,buffer,strlen(buffer),0);
        ...
        if (1 <= menu && lpMenu->menuCount >= menu) {
        	if (NULL != menuItem[menu-1].fnCommand) {
        		menuItem[menu-1].fnCommand(sd,userFullList,chatUser);
        	} else {
